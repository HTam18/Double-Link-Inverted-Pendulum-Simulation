function [rail_force, info] = dip_rail_limit(x, x_dot, params)
% DIP_RAIL_LIMIT Soft rail limit force for the cart x axis.
%
% Phase 23 purpose:
%   - Model finite cart travel on a rail without creating an infinite hard wall.
%   - Add a smooth inward force when the cart is close to, or beyond, x_min/x_max.
%   - Keep the solver stable by saturating the wall force.
%
% State convention:
%   x     = cart position [m]
%   x_dot = cart velocity [m/s]
%
% Parameter fields, normally from shared/parameters.json:
%   params.rail_limit.x_min_m
%   params.rail_limit.x_max_m
%   params.rail_limit.x_soft_m
%   params.rail_limit.k_wall_N_per_m
%   params.rail_limit.c_wall_N_s_per_m
%   params.rail_limit.max_wall_force_N
%
% Sign convention:
%   Positive rail_force pushes the cart to the right.
%   Negative rail_force pushes the cart to the left.
    ...
    if x_min >= x_max
        error('dip_rail_limit:InvalidRailRange', 'x_min_m must be smaller than x_max_m.');
    end
    if x_soft < 0 || k_wall < 0 || c_wall < 0 || max_wall_force <= 0
        error('dip_rail_limit:InvalidRailParams', ...
    ...
    force_left = 0.0;
    if left_active
        % Push right. Damping only opposes outward motion toward the left.
        force_left = k_wall * left_penetration - c_wall * min(double(x_dot), 0.0);
    end

    force_right = 0.0;
    if right_active
        % Push left. Damping only opposes outward motion toward the right.
    ...
    if ~isfinite(rail_force)
        error('dip_rail_limit:NonFiniteForce', 'rail_force became NaN or Inf.');
    end
