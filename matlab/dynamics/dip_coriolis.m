function C = dip_coriolis(theta1, theta2, q_dot, params)
% DIP_CORIOLIS Return velocity-dependent nonlinear terms for q = [x, theta1, theta2].
%
...
%   theta = 0 rad means downward, theta = pi rad means upright.
    ...
    q_dot = q_dot(:);
    if numel(q_dot) ~= 3
        error('dip_coriolis:InvalidQDot', 'q_dot must have 3 elements.');
    end
    ...
    for i = 1:3
        for j = 1:3
            for k = 1:3
                % MATLAB is 1-indexed. dM{idx} maps to derivative with respect to
                % q(idx), where q = [x, theta1, theta2].
