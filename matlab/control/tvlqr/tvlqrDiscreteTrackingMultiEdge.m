function tracker = tvlqrDiscreteTrackingMultiEdge(trajectory, params, user_cfg)
    ...
    cfg = RecoveryUtils.defaultCfg(user_cfg, params, trajectory);
    ...
    tracker = struct();
    tracker.type = 'discrete_tvlqr_multi_edge';
    tracker.workflow = 'transition_tracking';
    tracker.edge_name = local_get_text_field(trajectory, 'edge_name', 'unknown_edge');
    tracker.source_target = local_get_text_field(trajectory, 'source_target', 'unknown_source');
    tracker.destination_target = local_get_text_field(trajectory, 'destination_target', 'unknown_target');
    tracker.state_order = {'x', 'x_dot', 'theta1', 'theta1_dot', 'theta2', 'theta2_dot'};
    tracker.angle_convention = 'theta = 0 down, theta = pi up, theta2 absolute';
    tracker.control_law = 'u = u_ff(k) - K(k)*e(k), with wrapped angle error';
    tracker.t = ref.t;
    tracker.x_ref = ref.x;
    tracker.u_ref = ref.u_actual;
    tracker.u_cmd_ref = ref.u_actual;
    tracker.target_state = cfg.target_state(:);
    tracker.K = K;
    tracker.A = A;
    tracker.B = B;
    tracker.P = P;
...
function cfg = defaultCfg(user_cfg, params, trajectory)
    cfg = struct();
    ...
    cfg.R = 2.0;
    cfg.target_state = local_target_state_from_trajectory(trajectory, params);
...
function target_state = local_target_state_from_trajectory(trajectory, params)
    if isfield(trajectory, 'xf') && numel(trajectory.xf) == 6
        target_state = double(trajectory.xf(:));
        target_state(1) = 0;
        target_state(2) = 0;
        target_state(4) = 0;
        target_state(6) = 0;
    ...
    target_name = local_get_text_field(trajectory, 'destination_target', 'up_up');
    if isfield(params, 'target_modes') && isfield(params.target_modes, target_name)
        tm = params.target_modes.(target_name);
        target_state = [0; 0; double(tm.theta1_target_rad); 0; double(tm.theta2_target_rad); 0];
    else
...
function ref = local_build_reference(trajectory, cfg)
    ...
    if size(X0, 1) ~= 6
        error('tvlqrDiscreteTrackingMultiEdge:InvalidTrajectory', 'trajectory.x_ref must be N x 6.');
    end
    if numel(U0) ~= numel(t0) - 1
        error('tvlqrDiscreteTrackingMultiEdge:InvalidTrajectory', 'trajectory.u_ff length must be N-1.');
    end
...
function x_next = local_rk4_zoh_step(x, u, dt, n_substeps, params)
    ...
    for ii = 1:n_substeps
        f = @(xx) dipDynamicsNonlinear(xx, u, params);
        k1 = f(x_next);
...
function value = local_get_text_field(s, name, default_value)
    if isstruct(s) && isfield(s, name) && ~isempty(s.(name))
        value = char(s.(name));
    else
        value = default_value;
